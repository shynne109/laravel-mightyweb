...
new class extends Component {
    ...
    public function updatedPosition(): void
    {
        ...
        $maxOrder = NavigationIcon::where('position', $this->position)->max('id');
...
<div class="p-6">
    ...
    <div class="mb-6 flex flex-col sm:flex-row gap-4">
        <div class="flex-1">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="Search by title or action..."
                icon="magnifying-glass" />
        </div>
        <div class="flex items-center gap-4">
            <flux:select wire:model.live="filterPosition" placeholder="Position">
                <flux:select.option value="all">All Positions</flux:select.option>
                <flux:select.option value="left">Left Icons</flux:select.option>
                <flux:select.option value="right">Right Icons</flux:select.option>
            </flux:select>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if ($icon->icon)
                            <img src="{{ asset('storage/' . $icon->icon) }}" alt="{{ $icon->title }}"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <flux:badge size="sm" :color="$icon->position === 'left' ? 'primary' : 'purple'"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <flux:badge wire:click="toggleActive({{ $icon->id }})" size="sm"
                                :color="$icon->is_active ? 'green' : 'gray'" class="cursor-pointer">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <flux:button icon="pencil" size="sm" variant="ghost"
                                    wire:click="openEditModal({{ $icon->id }})" />
                                <flux:button icon="trash" size="sm" variant="ghost" color="red"
                                    wire:click="confirmDelete({{ $icon->id }})" />
                            </div>
                        </td>
                    </tr>
                    @empty
    ...
    <flux:modal name="confirmingDelete" class="md:w-96">
        <form wire:submit="delete" class="space-y-6">
            <div>
                ...
                <flux:subheading>
                    <p class="mt-2">Are you sure you want to delete this navigation icon? This action cannot be undone.
                    </p>
                </flux:subheading>
            </div>
    ...
    <flux:modal variant="flyout" name="showCreateModal" class="md:w-[600px]">
        <form wire:submit="save" class="space-y-6">
            ...
            <flux:input wire:model.defer="title" label="Icon Title" name="title" placeholder="Enter icon title" />

            <flux:input wire:model.defer="action" label="Action/URL" name="action"
                placeholder="https://example.com or action name"
                description="The URL or action to trigger when icon is clicked" />

            <flux:input type="file" wire:model="icon" label="Icon Image" name="icon" accept="image/*"
                description="Upload an icon image (max 2MB)" />

            <div class="grid grid-cols-3 gap-4">
                <flux:select wire:model="position" label="Position" name="position">
                    <flux:select.option value="left">Left</flux:select.option>
                    <flux:select.option value="right">Right</flux:select.option>
                </flux:select>

                <flux:input wire:model.defer="sort_order" label="Sort Order" name="sort_order" type="number" min="0" />
